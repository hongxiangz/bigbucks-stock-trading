...
def calculate_sharpe_volatility(username, start_date, current_date):
    ...
    stocks=stock_names
    total_value = sum([stock[1] for stock in stocks])
    data = dict()
    current_weights = []
    ...
    for stock in stocks:
        stock_name = stock[0]
        stock_value = stock[1]
        stock_percentage = (stock_value / total_value)
        weights.append(stock_percentage)
        prt = prt + stock_percentage * each_ret[gj]
        gj = gj +1
    # prt is portfolio return
    weights = np.array(weights)
    data = pd.DataFrame.from_dict(data, orient='columns')
    re = data.pct_change()
    cov=np.array(re.cov())
    port_dev=np.dot(weights.T,np.dot(cov,weights))
    sigma=np.sqrt(port_dev)
    prts=(prt-0.03572)/sigma
    points, min_sharp, max_sharp = graph_EF(data)
    return prts,prt,sigma,points, min_sharp, max_sharp
